...
const AdminSoccerTable = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const { loading, soccerMatches = [] } = useSelector((state) => state.match) || {};
  ...
  // Time logic
  const now = new Date();
  const upcomingMatches = soccerMatches
    .filter((match) => {
  ...
  const liveMatches = soccerMatches
    .filter((match) => {
      const matchTime = new Date(match.openDate);
      return (
        matchTime <= now &&
  ...
  const handleSelectChange = async (eventId, selected) => {
    ...
    // Optionally: dispatch(getSoccerMatches());
  };
  ...
  if (!upcomingMatches.length && !liveMatches.length) return <div>No soccer matches found.</div>;
  ...
  return (
    <>
      <Helmet>
        <title>Admin Soccer Matches | betinfo.live</title>
        <meta name="description" content="Manage, approve, and update soccer match statuses and odds on betinfo.live" />
      </Helmet>
      <div className="container mt-4 ">
        <h1 className="mb-4">Soccer: Upcoming and Live Matches</h1>
...
export default AdminSoccerTable;
