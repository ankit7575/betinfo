...
const AdminTennisTable = () => {
  ...
  // Time logic
  const now = new Date();
  const upcomingMatches = tennisMatches
    .filter((match) => {
  ...
  const liveMatches = tennisMatches
    .filter((match) => {
      const matchTime = new Date(match.openDate);
      return (
        matchTime <= now &&
  ...
  const handleSelectChange = async (eventId, selected) => {
    ...
    // Optionally: dispatch(getTennisMatches());
  };
  ...
  if (!upcomingMatches.length && !liveMatches.length) return <div>No valid matches found</div>;
  ...
  return (
    <>
      ...
      <div className="container mt-4 ">
        <h1 className="mb-4">Tennis: Upcoming and Live Matches</h1>
